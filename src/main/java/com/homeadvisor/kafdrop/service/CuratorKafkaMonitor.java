...
public class CuratorKafkaMonitor implements KafkaMonitor {
        ...
        topicVO.ifPresent(
                vo -> {
                    getTopicPartitionSizes(vo, kafka.api.OffsetRequest.LatestTime())
                            .entrySet()
                            .forEach(entry -> vo.getPartition(entry.getKey())
                                    .ifPresent(p -> p.setSize(entry.getValue())));
                    getTopicPartitionSizes(vo, kafka.api.OffsetRequest.EarliestTime())
                            .entrySet()
                            .forEach(entry -> vo.getPartition(entry.getKey())
                                    .ifPresent(p -> p.setFirstOffset(entry.getValue())));
                });
        return topicVO;
    }
    ...
    public TopicVO parseZkTopic(ChildData input) {
        try {
            ...
            topic.setConfig(
                    Optional.ofNullable(
                            topicConfigPathCache.getCurrentData(ZkUtils.ConfigPath() + "/" + topic.getName()))
    ...
    private Map<String, TopicVO> getTopicMetadata(BlockingChannel channel, String... topics) {
        final TopicMetadataRequest request = new TopicMetadataRequest((short) 0, 0, clientId(), Arrays.asList(topics));
    ...
    public Optional<ConsumerVO> getConsumerByTopic(String groupId, TopicVO topic) {
        ...
        Optional.ofNullable(consumerTreeCache.getCurrentChildren(groupDirs.consumerRegistryDir()))
                .ifPresent(
                        children -> children.keySet().stream()
        ...
        if (topicOpt != null) {
            ...
            return topic.getPartitions().stream()
                    .map(partition -> {
                        ...
                        consumerPartition
                                .setFirstOffset(topicPartition.map(TopicPartitionVO::getFirstOffset).orElse(-1L));
     ...
     *
     * @param channel
     *            The channel to send requests on
     * @param groupId
     *            Consumer group to use
     * @param topic
     *            Topic to query
     * @param zookeeperOffsets
     *            If true, use a version of the API that retrieves
    ...
    private Map<Integer, Long> getConsumerOffsets(BlockingChannel channel,
        ...
        final OffsetFetchRequest request = new OffsetFetchRequest(
                ...
                topic.getPartitions().stream()
                        ...
                        .collect(Collectors.toList()),
                (short) (zookeeperOffsets ? 0 : 1), 0, // version 0 = zookeeper offsets, 1 = kafka
                                                       // offsets
                clientId());
    ...
    /**
     * Returns the broker Id that is the offset coordinator for the given group id. If not found,
     * returns null
     */
    ...
    private Integer offsetManagerBroker(BlockingChannel channel, String groupId) {
        final GroupCoordinatorRequest request = new GroupCoordinatorRequest(groupId, (short) 0, 0, clientId());
        ...
        return (response.error().code() == ErrorMapping.NoError()) ? response.coordinatorOpt().get().id() : null;
    }
    ...
    private Map<Integer, Long> getTopicPartitionSizes(TopicVO topic, long time) {
        try {
            ...
            return threadPool.submit(() -> topic.getPartitions().parallelStream()
                    .filter(p -> p.getLeader() != null)
                    .collect(Collectors.groupingBy(p -> p.getLeader().getId())) // Group partitions
                                                                                // by leader broker
                                                                                // id
    ...
    private OffsetResponse sendOffsetRequest(Integer brokerId, TopicVO topic,
        ...
        return retryTemplate.execute(
                context -> brokerChannel(brokerId)
                        .execute(channel -> {
